...
function Mobiles() {
	...
	const handleCheckboxChange = (event) => {
		...
		if (event.target.checked) {
			...
			setList(list.filter((i) => i !== itemValue));
		}
	};

	//http://localhost:3000/search?searchQuery=vivo
	//http://localhost:3000/filter?brand=Vivo,Samsung
	const queryString = list
		.map((item) => `${encodeURIComponent(item)}`)
		.join(",");

	const url = `/filter?brand=${queryString}`;
